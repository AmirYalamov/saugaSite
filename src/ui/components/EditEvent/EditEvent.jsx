...
export default function Locations(props){
  ...
  React.useEffect(()=>{
    ...
    .then((response) => {
      ...
      if(response.data._id){
        ...
        setDate(response.data.start_date)
        setEndTime(response.data.end_date)
        setStartTime(response.data.start_date)
        // For location and event, will have to find by ID
        const locationID = response.data.location._id
        axios.get('/v1/locations/'+locationID).then((locationResponse) =>{
          // console.log(locationResponse.data.name)
          setLocationName(locationResponse.data.name)
        }).catch((e) => console.log(e))
        const eventTypeID = response.data.event_type._id
        axios.get('/v1/event_types/'+eventTypeID).then((eventTypeResponse) => {
          // console.log(eventTypeResponse.data)
          setEventTypeName(eventTypeResponse.data.name)
        }).catch((e) => console.log(e))
      }
    })
    .catch((error) => {
  ...
  const saveAndContinue = (e) => {
    ...
    const body = {
      ...
      end_date: endTime,
      image_url: 'https://i.cbc.ca/1.5809122.1605830847!/fileImage/httpImage/image.jpg_gen/derivatives/16x9_780/custom-fit-private-gym-edmonton.jpg',
      capacity: capacity,
    }

    axios.patch('/v1/events/'+eventData._id, body)
    .then((response) => {
      console.log(response.data)
    })
    .catch((error) => {
